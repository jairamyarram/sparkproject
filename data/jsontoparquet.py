...
def process_json_to_parquet():
    try:
        ...
        if df.rdd.isEmpty():
            message_body = f"Alert: The file '{json_file_path}' was empty. No data was processed or converted to Parquet."
            print(message_body)
            send_email("PySpark Job Failed: No Data Received", message_body)
        else:
            # --- Convert to Parquet and save ---
            df.coalesce(1).write.parquet(parquet_output_path, mode="overwrite")
            message_body = f"Success: The file '{json_file_path}' was successfully converted to Parquet.\nThe output is at '{parquet_output_path}'."
            print(message_body)
